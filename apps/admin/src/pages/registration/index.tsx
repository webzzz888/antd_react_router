...
import { deleteRegistration, updateRegistration, addRegistration, getRegistrationList } from 'apis';
...
function Registration() {
  ...
  const initUserData = async () => {
    try {
      const res: any = await getRegistrationList(searchData);
  ...
  const handleDeleteUser = async (ids: any) => {
    try {
      await deleteRegistration(ids);
      message.success('删除人员登记成功！');
      initUserData();
      if (ids == selectedRowKeys) {
        setSelectedRowKeys([]);
      }
    } catch (error) {
      message.error('删除人员登记失败！');
    }
  };
  ...
  const resetData = () => {
    if (searchData.yonghuming || searchData.renyuanbianhao) {
    ...
    <>
      ...
      <Form name="basic" onFinish={onFinish} autoComplete="off" size="large">
        <Row gutter={24}>
          ...
          <Col span={8}>
            <Form.Item label="人员编号" name="renyuanbianhao" wrapperCol={{ span: 18 }}>
              <Input placeholder="请输入人员编号查询" />
            </Form.Item>
          </Col>
      ...
      >
        ...
        >
          ...
          >
            <InputNumber placeholder="请输入年龄（1,130）" style={{ width: '100%' }} min={1} max={130}/>
          </Form.Item>
          <Form.Item
            name="lianxidianhua"
            label="联系电话"
